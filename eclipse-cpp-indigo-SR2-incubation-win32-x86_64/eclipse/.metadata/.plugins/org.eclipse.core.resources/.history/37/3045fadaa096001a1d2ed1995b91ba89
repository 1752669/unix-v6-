...
void CRT::WriteChar(char ch,int ntty)
{
	if(ntty == 0)
	{
		m_VideoMemory[m_CursorY * CRT::COLUMNS + m_CursorX] = (unsigned char) ch | 0x0A00;
		m_CursorX++;
...
void CRT::ClearScreen(int ntty)
{
	int  i = 0;
	int j = 0;
	for(j = 0; j<CRT::ROWS;j++)
	{	if(ntty == 0)
		for ( i = 0; i <  variable; i++ )
		{
			m_VideoMemory[j*COLUMNS+i] = (unsigned short)' ' | 0x0A00;
		}
	else for( i = variable; i < CRT::COLUMNS; i++)
		{
